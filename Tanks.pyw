...
while run[0]:
    ...
    helicopter.velocity = vec(speed + speedH, .5).rotate(helicopter.angle)
    ...
    if (key[pg.K_SPACE] or salvo == 1) and tank1.position.x >= tank1_pos and not salvoT:
        ...
        fire.velocity = vec(projectile_velocity, 0).rotate(fire.angle)
        ...
        salvo = 0
        soundT1.play()
    if not pg.sprite.collide_rect(fire, barrel) and collide_fire:
        all_sprites.add(fire)
        all_sprites.change_layer(fire, 2)
        fire.update()
        time_fire1 = 0
        collide_fire = False
        counting = True
    ...
    if tank2.position.x > WIDTH_WIN:
        ...
        if time_fire2 == 0:
            ...
            all_sprites.change_layer(fire2, 1)
            ...
            fire2.velocity = vec(-projectile_velocity, 0).rotate(fire2.angle)
    ...
    "Стрельба вертолета"
    if helicopter.position.x < WIDTH_WIN and not expH:
    ...
    if pg.sprite.collide_mask(shell, tank2):
        # print('снаряд - T2')
        count['killed'] += 1
        count['life'] += 0.1
        shell.velocity.x, shell.velocity.y = 0, 0
        ...
        all_sprites.change_layer(explosion, 5)
        expT = True
        ...
        if explosion.scale < 0.2:
            ...
            all_sprites.remove(explosion)
            salvoT = False
    elif pg.sprite.collide_mask(tank1, helicopter):
        # print('H-T1')
        count['life'] -= 1.
        start()
        speedH = random.randint(-1, 1)
        ...
        all_sprites.change_layer(explosion2, 5)
