...
def load_images(path: dict) -> dict:
    dictionary_images = {}
    for npath in os.listdir(path):
        list_images = []
        for file_name in sorted(os.listdir(os.path.join(path, npath))):
            list_images.append(pg.image.load(os.path.join(path, npath, file_name)))
        dictionary_images[npath] = list_images
...
dict_images = load_images(path_images)
