...
def icon():
    ...
    font_text = pgm.SysFont('Arial', 16, True)
...
images0 = load_images(path=os.path.join(mainpath, 'Image', 'Earth'))
...
images4 = load_images(path=os.path.join(mainpath, 'Image', 'Tank2'))
...
images5 = load_images(path=os.path.join(mainpath, 'Image', 'Дуло2'))
...
images6 = load_images(path=os.path.join(mainpath, 'Image', 'Прицел'))
sight = Sprite(x=0, y=0, dx=False, dy=False, images=images6, angle=False, scale=0.5)

images7 = load_images(path=os.path.join(mainpath, 'Image', 'Снаряд'))
...
images8 = load_images(path=os.path.join(mainpath, 'Image', 'Взрыв'))
...
images9 = load_images(path=os.path.join(mainpath, 'Image', 'fire'))
...
images10 = load_images(path=os.path.join(mainpath, 'Image', 'Bullet'))
bullet = Sprite(x=helicopter.position.x, y=helicopter.position.y, dx=False, dy=False,
                images=images10, angle=0, scale=1.0)

images11 = load_images(path=os.path.join(mainpath, 'Image', 'Здоровье'))
health = Health(images=images11)

images12 = load_images(path=os.path.join(mainpath, 'Image', 'Mouse'))
...
while run[0]:
    ...
    if pg.sprite.spritecollide(tank2, shell_box, True):
        ...
        expT = True
    elif pg.sprite.spritecollide(helicopter, shell_box, True, pg.sprite.collide_circle_ratio(.5)):
        # print('снаряд - H')
        count['killed'] += 1
        count['life'] += 0.1
        all_sprites.add(explosion, layer=4)
    ...
    info_string.blit(text_font.render(f"Points: {count['killed']}", True, (255, 255, 255)), (10, 2))
    info_string.blit(text_font.render(f"Life: {int(count['life'])}", True, (255, 255, 255)), (868, 2))
