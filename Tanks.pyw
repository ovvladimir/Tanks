...
def menu():
    ...
    while run[1]:
        ...
        if tx < tx_min:
            tx = WIDTH_WIN
        if color_list[col] >= 250 or color_list[col] <= 0:
            col = 0
            color_list.reverse()
        col += 1
        ...
        for ev in pg.event.get():
            if ev.type == pg.QUIT:
                ...
                if ev.key == pg.K_ESCAPE:
                    ...
                    and (WIDTH_WIN - font_size[0]) / 2. < m_pos[0] < (WIDTH_WIN + font_size[0]) / 2.:
                if point == 0:
                    menu_box.empty()
                    start()
                    pg.mouse.set_visible(False)
        ...
        if fire_block:
            screen.blit(font_menu.render('GAME OVER', True, (color_list[col], 250, 250)), (tx, 580))
        else:
            screen.blit(font_menu.render('TANKS', True, (color_list[col], 250, 250)), (tx, 580))
...
shell = Sprite(x=-WIDTH_WIN, y=0, dx=False, dy=False, images=images7, angle=0, scale=0.4)
...
vol_img = pg.transform.scale(vol_img, (int(vol_img.get_width() / 1.5), int(vol_img.get_height() / 1.5)))
